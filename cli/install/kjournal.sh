...
setup_tmp() {
    TMP_DIR=$(mktemp -d -t kjournal-install.XXXXXXXXXX)
    TMP_METADATA="${TMP_DIR}/kjournal.json"
    TMP_HASH="${TMP_DIR}/kjournal.hash"
    TMP_BIN="${TMP_DIR}/kjournal.tar.gz"
    cleanup() {
        local code=$?
        set +e
        trap - EXIT
